...
void Ident:: operator= (const Ident & ident)
{
	name = new char [ strlen (ident.name) ];
	strcpy (name, ident.name);
...
/*
template <class T>
int Table <T>:: put (const char * buf)
{
	if ( top = size - 1 ) {
		extend_table ();
	}
